...
defmodule OMG.ChildChain.Integration.HappyPathTest do
  ...
  test "deposit, spend, restart, exit etc works fine", %{
    ...
    {:ok, _} = OMG.Eth.Test.Support.DevHelper.wait_for_next_child_block(post_spend_child_block)
    ...
    {:ok, _} = OMG.Eth.Test.Support.DevHelper.wait_for_next_child_block(post_spend_child_block2)
    ...
    {:ok, _} = OMG.Eth.Test.Support.DevHelper.wait_for_root_chain_block(exit_eth_height + exiters_finality_margin)
  ...
  test "check that unspent funds can be exited exited with in-flight exits",
    ...
    exiters_finality_margin = Application.fetch_env!(:omg, :deposit_finality_margin) + 1
    OMG.Eth.Test.Support.DevHelper.wait_for_root_chain_block(eth_height + exiters_finality_margin)
