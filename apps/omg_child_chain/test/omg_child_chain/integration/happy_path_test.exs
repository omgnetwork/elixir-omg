...
defmodule OMG.ChildChain.Integration.HappyPathTest do
  ...
  test "deposit, spend, restart, exit etc works fine", %{
    ...
    raw_txbytes = Transaction.Extract.raw_txbytes(raw_tx2)
  ...
  test "check that unspent funds can be exited exited with in-flight exits",
    ...
    {:ok, %{"status" => "0x1", "blockNumber" => eth_height}} =
      Eth.RootChainHelper.in_flight_exit(
        in_flight_tx |> Transaction.Extract.raw_txbytes(),
        get_input_txs([tx, tx]),
        proof <> proof,
    ...
    in_flight_tx2_rawbytes = in_flight_tx2 |> Transaction.Extract.raw_txbytes()
  ...
  defp get_input_txs(txs) do
    txs
    |> Enum.map(&(Transaction.Extract.raw_txbytes(&1) |> ExRLP.decode()))
    |> ExRLP.encode()
  end
end
