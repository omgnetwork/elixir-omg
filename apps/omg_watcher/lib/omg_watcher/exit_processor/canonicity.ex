...
defmodule OMG.Watcher.ExitProcessor.Canonicity do
  ...
  def get_ife_txs_with_competitors(%Core{in_flight_exits: ifes}, known_txs_by_input) do
    ...
    |> Stream.map(fn {ife, _double_spend} -> OMG.Transaction.Extract.raw_txbytes(ife.tx) end)
    |> Enum.uniq()
  ...
  def get_invalid_ife_challenges(%Core{in_flight_exits: ifes}) do
    ...
    |> Stream.map(&OMG.Transaction.Extract.raw_txbytes(&1.tx))
    |> Enum.uniq()
  ...
  @spec get_competitor_for_ife(ExitProcessor.Request.t(), Core.t(), binary()) ::
          ...
          | {:error, OMG.Transaction.Decoding.decode_error()}
  def get_competitor_for_ife(
    ...
    with {:ok, ife_tx} <- OMG.Transaction.Decoding.decode(ife_txbytes),
  ...
  def prove_canonical_for_ife(%Core{} = state, ife_txbytes) do
    with {:ok, raw_ife_tx} <- OMG.Transaction.Decoding.decode(ife_txbytes),
  ...
  defp prepare_competitor_response(
    ...
    %{
      in_flight_txbytes: signed_ife_tx |> OMG.Transaction.Extract.raw_txbytes(),
      in_flight_input_index: in_flight_input_index,
      competing_txbytes: known_signed_tx |> OMG.Transaction.Extract.raw_txbytes(),
  ...
  defp prepare_canonical_response(%InFlightExitInfo{tx: tx, tx_seen_in_blocks_at: {pos, proof}}),
    do: %{in_flight_txbytes: OMG.Transaction.Extract.raw_txbytes(tx), in_flight_tx_pos: pos, in_flight_proof: proof}
