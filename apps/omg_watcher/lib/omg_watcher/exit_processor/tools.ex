...
defmodule OMG.Watcher.ExitProcessor.Tools do
  ...
  def double_spends_from_known_tx(inputs, %KnownTx{signed_tx: signed} = known_tx) when is_list(inputs) do
    known_spent_inputs = signed |> Transaction.Extract.get_inputs() |> Enum.with_index()
  ...
  def txs_different(tx1, tx2), do: Transaction.Extract.raw_txhash(tx1) != Transaction.Extract.raw_txhash(tx2)

  def get_ife(ife_tx, ifes) do
    case ifes[Transaction.Extract.raw_txhash(ife_tx)] do
