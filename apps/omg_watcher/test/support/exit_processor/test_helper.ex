...
defmodule OMG.Watcher.ExitProcessor.TestHelper do
  ...
  def se_event_status(tx, exiting_pos, opts \\ []) do
    ...
    owner = tx |> OMG.Transaction.Extract.get_outputs() |> Enum.at(oindex) |> Map.get(:owner)
  ...
  def ife_event(tx, opts \\ []) do
    ...
    %{
      call_data: %{in_flight_tx: OMG.Transaction.Extract.raw_txbytes(tx), in_flight_tx_sigs: Enum.join(sigs)},
      eth_height: eth_height
    }
  end

  def ife_response(tx, position),
    do: %{tx_hash: OMG.Transaction.Extract.raw_txhash(tx), challenge_position: Utxo.Position.encode(position)}
  ...
  def txbytes(tx), do: OMG.Transaction.Extract.raw_txbytes(tx)
