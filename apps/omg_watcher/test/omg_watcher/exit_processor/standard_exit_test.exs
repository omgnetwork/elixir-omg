...
defmodule OMG.Watcher.ExitProcessor.StandardExitTest do
  ...
  describe "Core.determine_exit_txbytes" do
    test "produces valid exit txbytes for exits from deposits",
      ...
      deposit_txbytes = OMG.Transaction.Extract.raw_txbytes(tx)
    ...
    test "produces valid exit txbytes for exits from txs in child blocks",
      ...
      creating_txbytes = OMG.Transaction.Extract.raw_txbytes(creating_recovered)
  ...
  defp get_bytes_sig(tx, sig_idx \\ 0),
    do: {OMG.Transaction.Extract.raw_txbytes(tx), Enum.at(tx.signed_tx.sigs, sig_idx)}
end
