...
defmodule OMG.Watcher.ExitProcessor.FinalizationsTest do
  ...
  @utxo_pos1 %OMG.InputPointer{blknum: 2_000, txindex: 0, oindex: 0}
  @exit_id 1
  ...
  describe "determining utxos that are exited by finalization" do
    test "signals all included txs' outputs as exiting when piggybacked output exits",
      ...
      tx1_first_output = %OMG.InputPointer{blknum: tx1_blknum, txindex: 0, oindex: 0}
      tx1_second_output = %OMG.InputPointer{blknum: tx1_blknum, txindex: 0, oindex: 1}
    ...
    test "returns utxos that should be spent when exit finalizes, two ifes combined",
      ...
      tx2_first_output = %OMG.InputPointer{blknum: tx2_blknum, txindex: 1, oindex: 0}
