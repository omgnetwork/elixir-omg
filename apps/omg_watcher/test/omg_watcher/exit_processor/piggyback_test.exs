  ...
  describe "available piggybacks" do
    ...
    test "transaction with different input/output owners",
      ...
      assert {:ok,
              [
               ...
               check_validity_filtered(%ExitProcessor.Request{blknum_now: 1000, eth_timestamp_now: :os.system_time(:second) +  5}, processor,
    ...
    test "challenged IFEs emit the same piggybacks as canonical ones",
      ...
      assert {:ok, events_canonical} =
               Core.check_validity(%ExitProcessor.Request{blknum_now: 1000, eth_timestamp_now: :os.system_time(:second) +  5}, processor)

      {challenged_processor, _} = Core.new_ife_challenges(processor, [ife_challenge(tx, comp)])

      assert {:ok, events_challenged} =
               Core.check_validity(%ExitProcessor.Request{blknum_now: 5000, eth_timestamp_now: :os.system_time(:second) +  5}, challenged_processor)
