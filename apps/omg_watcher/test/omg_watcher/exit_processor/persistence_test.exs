...
defmodule OMG.Watcher.ExitProcessor.PersistenceTest do
  ...
  setup %{db_pid: db_pid} do
    ...
    [txbytes1, txbytes2] = Enum.map(transactions, &Transaction.Extract.raw_txbytes/1)
  ...
  test "persist new challenges, responses and piggybacks",
    ...
    challenge = %{
      ...
      call_data: %{
        competing_tx: Transaction.Extract.raw_txbytes(competing_tx),
  ...
  defp persist_new_ifes(processor, txs, priv_keys, statuses \\ nil, db_pid) do
    ...
    encoded_txs = Enum.map(txs, &Transaction.Extract.raw_txbytes/1)
