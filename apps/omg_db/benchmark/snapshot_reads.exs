...
#
# Usage: mix run apps/omg_db/benchmark/snapshot_reads.exs
#
...
Benchee.run(
  %{
    "utxos" => fn {_dir, db_ref, _load_generator_pid} ->
      {:ok, _utxos} =
        db_ref
        |> OMG.DB.RocksDB.Core.filter_keys(:utxo)
        |> OMG.DB.RocksDB.Core.decode_values()
    end,
    "utxos_with_snapshot" => fn {_dir, db_ref, _load_generator_pid} ->
      {:ok, snapshot_ref} = :rocksdb.snapshot(db_ref)

      # The following 3 lines are OMG.DB.RocksDB.Core.do_filter_keys/2 modified to support snapshot
