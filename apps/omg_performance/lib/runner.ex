...
defmodule OMG.Performance.Runner do
  ...
  def run(ntx_to_send, nusers, opt) do
    {duration, _result} =
      :timer.tc(fn ->
        ...
        manager = OMG.Performance.SenderManager.start_link_all_senders(ntx_to_send, nusers, opt)

        # fire block creator
        _ = OMG.Performance.BlockCreator.start_link(opt[:block_every_ms])
  ...
  def profile_and_run(ntx_to_send, nusers, opt) do
    :fprof.apply(&OMG.Performance.Runner.run/3, [ntx_to_send, nusers, opt], procs: [:all])
    :fprof.profile()
