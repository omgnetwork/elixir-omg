...
defmodule OMG.WatcherInformational.ReleaseTasks.SetTracer do
  @moduledoc false
  ...
  def init(_args) do
    ...
    config = Application.get_env(@app, OMG.WatcherInformational.Tracer)
    config = Keyword.put(config, :disabled?, get_dd_disabled())
    config = Keyword.put(config, :env, get_app_env())
    :ok = Application.put_env(@app, OMG.WatcherInformational.Tracer, config, persistent: true)
  end

  defp get_dd_disabled do
    dd_disabled? = validate_bool(get_env("DD_DISABLED"), Application.get_env(@app, OMG.WatcherInformational.Tracer)[:disabled?])
  ...
  defp get_app_env do
    env = validate_string(get_env("APP_ENV"), Application.get_env(@app, OMG.WatcherInformational.Tracer)[:env])
