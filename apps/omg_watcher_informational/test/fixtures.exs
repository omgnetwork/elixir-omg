...
defmodule OMG.WatcherInformational.Fixtures do
  ...
  deffixture mix_based_child_chain(contract, fee_file) do
    ...
    exexec_opts_for_mix = [
      stdout: :stream,
      cd: Application.fetch_env!(:omg_watcher_informational, :umbrella_root_dir),
  ...
  deffixture in_beam_watcher(db_initialized, root_chain_contract_config) do
    ...
    {:ok, started_watcher} = Application.ensure_all_started(:omg_watcher_informational)
  ...
  deffixture web_endpoint do
    ...
    on_exit(fn ->
      WatcherInformationalHelper.wait_for_process(pid)
      :ok
    end)
  end
  ...
  deffixture phoenix_ecto_sandbox(web_endpoint) do
    ...
    {:ok, pid} =
      Supervisor.start_link(
        ...
        name: WatcherInformational.Supervisor
      )
  ...
  deffixture test_server do
    ...
    real_addr = Application.fetch_env!(:omg_watcher_informational, :child_chain_url)
    old_client_env = Application.fetch_env!(:omg_watcher_informational, :child_chain_url)
    fake_addr = "http://#{env.ip}:#{env.port}"

    on_exit(fn ->
      Application.put_env(:omg_watcher_informational, :child_chain_url, old_client_env)
