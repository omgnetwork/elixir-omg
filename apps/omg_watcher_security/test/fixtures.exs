...
defmodule OMG.WatcherSecurity.Fixtures do
  ...
  deffixture mix_based_child_chain(contract, fee_file) do
    ...
    exexec_opts_for_mix = [
      stdout: :stream,
      cd: Application.fetch_env!(:omg_watcher_security, :umbrella_root_dir),
  ...
  deffixture in_beam_watcher(db_initialized, root_chain_contract_config) do
    ...
    {:ok, started_security_watcher} = Application.ensure_all_started(:omg_watcher_security)
  ...
  deffixture test_server do
    ...
    real_addr = Application.fetch_env!(:omg_watcher_security, :child_chain_url)
    old_client_env = Application.fetch_env!(:omg_watcher_security, :child_chain_url)
    fake_addr = "http://#{env.ip}:#{env.port}"

    on_exit(fn ->
      Application.put_env(:omg_watcher_security, :child_chain_url, old_client_env)
