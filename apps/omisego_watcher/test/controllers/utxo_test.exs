defmodule OmiseGOWatcherWeb.Controller.UtxoTest do
  ...
  describe "UTXO database." do
    ...
    test "Consumed block contents are available.", %{alice: alice} do
      UtxoDB.consume_block(%Block{
        transactions: [
          API_Helper.create_signed([], @eth, [{alice, 1947}]),
          API_Helper.create_signed([], @eth, [{alice, 1952}])
        ],
        number: 2
      })
    ...
    test "Spent utxos are moved to new owner.", %{alice: alice, bob: bob, carol: carol} do
      UtxoDB.consume_block(%Block{
        transactions: [
          API_Helper.create_signed([], @eth, [{alice, 1843}]),
          API_Helper.create_signed([], @eth, [{bob, 1871}])
        ],
        number: 1
      })
      ...
      UtxoDB.consume_block(%Block{
        transactions: [API_Helper.create_signed([{1, 1, 0, bob}], @eth, [{carol, 1000}])],
        number: 2
      })
    ...
    test "Deposit utxo are moved to new owner if spent ", %{alice: alice, bob: bob} do
      ...
      UtxoDB.consume_block(%Block{
        transactions: [API_Helper.create_signed([{1, 0, 0, alice}], @eth, [{bob, 1}])],
        number: 2
      })
  ...
  test "compose_utxo_exit should return proper proof format", %{alice: alice} do
    TransactionDB.insert(API_Helper.create_signed([{1, 1, 0, alice}], @eth, [{alice, 120}]), 1, 1)
    TransactionDB.insert(API_Helper.create_signed([{1, 1, 0, alice}], @eth, [{alice, 110}]), 1, 2)
    TransactionDB.insert(API_Helper.create_signed([{2, 0, 0, alice}], @eth, []), 1, 3)
  ...
  test "compose_utxo_exit should return error when there is no tx in specfic block", %{alice: alice} do
    TransactionDB.insert(API_Helper.create_signed([{1, 0, 0, alice}], @eth, []), 1, 2)
    TransactionDB.insert(API_Helper.create_signed([{1, 1, 0, alice}], @eth, []), 1, 2)
    TransactionDB.insert(API_Helper.create_signed([], @eth, []), 1, 3)
