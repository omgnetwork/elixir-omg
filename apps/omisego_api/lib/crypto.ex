defmodule OmiseGO.API.Crypto do
  ...
  @spec decode_address(String.t() | binary) :: {:ok, address_t} | {:error, :invalid_address}
  ...
  @spec encode_address(binary) :: {:ok, String.t()} | {:error, :bad_address_encoding}
  def encode_address(address) when byte_size(address) == 20, do: {:ok, "0x" <> Base.encode16(address, case: :lower)}
  def encode_address(_), do: {:error, :bad_address_encoding}
