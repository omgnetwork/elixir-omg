defmodule OmiseGO.API.Api.CoreTest do
  ...
  defp create_encoded(inputs, cur12, outputs) do
    signed_tx = TestHelper.create_signed(inputs, cur12, outputs)
  ...
  test "signed transaction is valid in all input zeroing combinations", %{
    ...
    parametrized_tester = fn {input1, input2, spender1, spender2} ->
      ...
      encoded_signed_tx = create_encoded([input1, input2], eth(), [{alice, 7}, {bob, 3}])
  ...
  test "encoded transaction is corrupt", %{alice: alice, bob: bob} do
    encoded_signed_tx = create_encoded([{1, 2, 3, alice}, {2, 3, 4, bob}], eth(), [{alice, 7}])
  ...
  test "transaction is not allowed to have no input and a sig", %{alice: alice, bob: bob} do
    no_input_tx1 = create_encoded([{0, 0, 0, alice}, {2, 3, 4, bob}], eth(), [{alice, 7}])
    no_input_tx2 = create_encoded([{1, 2, 3, alice}, {0, 0, 0, bob}], eth(), [{alice, 7}])
  ...
  test "transaction is never allowed to have 2 empty inputs", %{alice: alice} do
    double_zero_tx1 = create_encoded([{0, 0, 0, %{priv: <<>>}}, {0, 0, 0, %{priv: <<>>}}], eth(), [{alice, 7}])
    double_zero_tx2 = create_encoded([{0, 0, 0, alice}, {0, 0, 0, %{priv: <<>>}}], eth(), [{alice, 7}])
    double_zero_tx3 = create_encoded([{0, 0, 0, %{priv: <<>>}}, {0, 0, 0, alice}], eth(), [{alice, 7}])
    double_zero_tx4 = create_encoded([{0, 0, 0, alice}, {0, 0, 0, alice}], eth(), [{alice, 7}])
