defmodule OmiseGO.API.Api.CoreTest do
  ...
  test "encoded transaction is corrupt", %{alice: alice, bob: bob} do
    {encoded_signed_tx, _} = create_encoded([{1, 2, 3, alice}, {2, 3, 4, bob}], eth(), [{alice, 7}])
  ...
  test "transaction is not allowed to have input and empty sig", %{alice: alice, bob: bob} do
    ...
    {partial_signed_tx1, _} = TestHelper.create_signed([{1, 2, 3, alice}], eth(), [{alice, 7}])
    ...
    {partial_signed_tx2, _} =
      TestHelper.create_signed([{0, 0, 0, %{priv: <<>>}}, {1, 2, 3, alice}], eth(), [{alice, 7}])
  ...
  test "transactions with corrupt signatures don't do harm", %{alice: alice} do
    {full_signed_tx, _} = TestHelper.create_signed([{1, 2, 3, alice}], eth(), [{alice, 7}])
  ...
  test "transaction is not allowed to have no input and a sig", %{alice: alice, bob: bob} do
    {no_input_tx1, _} = create_encoded([{0, 0, 0, alice}, {2, 3, 4, bob}], eth(), [{alice, 7}])
    {no_input_tx2, _} = create_encoded([{1, 2, 3, alice}, {0, 0, 0, bob}], eth(), [{alice, 7}])
  ...
  test "transaction is never allowed to have 2 empty inputs", %{alice: alice} do
    {double_zero_tx1, _} = create_encoded([{0, 0, 0, %{priv: <<>>}}, {0, 0, 0, %{priv: <<>>}}], eth(), [{alice, 7}])
    {double_zero_tx2, _} = create_encoded([{0, 0, 0, alice}, {0, 0, 0, %{priv: <<>>}}], eth(), [{alice, 7}])
    {double_zero_tx3, _} = create_encoded([{0, 0, 0, %{priv: <<>>}}, {0, 0, 0, alice}], eth(), [{alice, 7}])
    {double_zero_tx4, _} = create_encoded([{0, 0, 0, alice}, {0, 0, 0, alice}], eth(), [{alice, 7}])
