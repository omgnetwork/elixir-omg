...
In `docs/api-specs` folder there are numbered markdown files which forms final spec. Whenever API has changed and specs needs to be updated it should be updated along with the feature and push to the development branch. 

## Preparation - if you need to generate specs yourself

### Fetch the branch
Specs page files are separated from the content of other development branches. It makes sens to just fetch only one branch to sepearated folder outside of this repo files.

```
# somewhere OUTSIDE of this repo root folder
# clone just `gh-pages` branch
> git clone git@github.com:omisego/elixir-omg.git --branch gh-pages --single-branch gh_pages
> cd gh_pages
> ls
```
You should see just website files: html, css ...

### Getting the Slate generator
Clone [Slate repo] to some other folder outside of this repo and just created gh-pages branch files.

```
> git clone git@github.com:lord/slate.git slate
> cd slate
> bundle install # you need ruby installed
```

You need to follow above instructions just once

## Generating the specs site

Make sure you've setup you environment first as [above](Getting the Slate generator)

First concat all specs files in appropriate order into one file from which Slate generates the website.
Navigate to the `slate` folder (see above)
```
> cat <this repo root>/docs/api_specs/*.md > ./source/index.html.md

# Run the Slate to generate static website files
> bundle exec middleman build --clean
```
Above will generate static website files in `slate/build` folder


You can review generated site locally running `bundle exec middleman server` and openning `http://localhost:4567/` in you web browser.

## Updating specs in gh-pages branch

Assuming you generated website as [above](Generating the specs site)
Navigate to your `gh_pages` folder
```
> copy -r <your slate folder>/build/* .

> git add --all
> git commit -m "specs: regenerate from <this repo current git-sha>"
> git push
```

New version should be visible at `https://omisego.github.io/elixir-omg/#introduction`.
Well done!
