...
- "BLOCK_SUBMIT_GAS_PRICE_STRATEGY" - The gas price strategy to use for block submission. Note that all strategies will still run, but only the one configured here will be used for actual submission. Defaults to `LEGACY`.
