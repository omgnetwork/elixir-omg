...
defmodule LoadTesting.Scenario.DoChildChainTransactions do
  @moduledoc """
  ...
  def do_childchain_transaction(session, sender, fee_wei) do
    ...
    |> Chaperon.Session.add_metric(
      {:call, {LoadTesting.Scenario.DoChildChainTransactions, "ChildChainAPI.Api.Transaction.submit"}},
      Timing.timestamp() - start
    )
  end

  defp submit_tx(session, {inputs, outputs, sender}) do
    {:ok, blknum, txindex} = LoadTesting.Utils.ChildChain.submit_tx(inputs, outputs, [sender])
