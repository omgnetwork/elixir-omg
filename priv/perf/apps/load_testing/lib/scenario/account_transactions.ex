...
defmodule LoadTesting.Scenario.AccountTransactions do
  @moduledoc """
  ...
  defp test_api_account_get_balance(session, sender) do
    ...
    add_metric(
      session,
      {:call, {LoadTesting.Scenario.AccountTransactions, '/account.get_balance'}},
  ...
  defp test_api_account_get_utxos(session, sender) do
    ...
    add_metric(
      session,
      {:call, {LoadTesting.Scenario.AccountTransactions, '/account.get_utxos'}},
      Timing.timestamp() - start
    )
  end
  ...
  defp test_api_account_get_transactions(session, sender) do
    ...
    add_metric(
      session,
      {:call, {LoadTesting.Scenario.AccountTransactions, '/account.get_transactions'}},
      Timing.timestamp() - start
    )
  end
  ...
  defp test_api_account_create_and_submit_transactions(session, sender) do
    ...
    session =
      add_metric(
        session,
        {:call, {LoadTesting.Scenario.AccountTransactions, '/transaction.create'}},
        Timing.timestamp() - start
      )
    ...
    session =
      add_metric(
        session,
        {:call, {LoadTesting.Scenario.AccountTransactions, '/transaction.submit_typed'}},
        Timing.timestamp() - start
      )
