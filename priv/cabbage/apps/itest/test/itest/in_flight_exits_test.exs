...
defmodule InFlightExitsTests do
  ...
  defgiven ~r/^"(?<entity>[^"]+)" deposits "(?<amount>[^"]+)" ETH to the root chain$/,
           ...
           state do
    get_balance(state[entity]) |> IO.inspect(label: "#{entity} deposits #{amount} ETH to the root chain")
  ...
  defthen ~r/^"(?<entity>[^"]+)" should have "(?<amount>[^"]+)" ETH on the child chain after finality margin$/,
          %{entity: entity, amount: amount},
          state do
    get_balance(state[entity]) |> IO.inspect(label: "#{entity} should have #{amount} ETH on the child chain after finality margin")
    %{address: address} = entity_state = state[entity]
  ...
  defgiven ~r/^Alice creates a transaction for "(?<amount>[^"]+)" ETH$/,
           %{amount: amount},
           state do
    get_balance(state["Alice"]) |> IO.inspect(label: "Alice creates a transaction for #{amount} ETH")
    amount = Currency.to_wei(amount)
  ...
  defand ~r/^Alice sends the most recently created transaction$/, _, state do
    get_balance(state["Alice"]) |> IO.inspect(label: "Alice sends the most recently created transaction")
  ...
  defand ~r/^Alice starts an in flight exit from the most recently created transaction$/, _, state do
    get_balance(state["Alice"]) |> IO.inspect(label: "Alice starts an in flight exit from the most recently created transaction")
  ...
  defgiven ~r/^"(?<entity>[^"]+)" verifies its in flight exit from the most recently created transaction$/,
           %{entity: entity},
           state do
    get_balance(state[entity]) |> IO.inspect(label: "#{entity} verifies its in flight exit from the most recently created transaction")
    exit_game_contract_address = state["exit_game_contract_address"]
  ...
  defgiven ~r/^Alice piggybacks output from her most recent in flight exit$/, _, state do
    get_balance(state["Alice"]) |> IO.inspect(label: "Alice piggybacks output from her most recent in flight exit")
    exit_game_contract_address = state["exit_game_contract_address"]
  ...
  defwhen ~r/^Alice piggybacks inputs from Bobs most recent in flight exit$/, _, state do
    get_balance(state["Alice"]) |> IO.inspect(label: "Alice piggybacks inputs from Bobs most recent in flight exit")
    exit_game_contract_address = state["exit_game_contract_address"]
  ...
  defand ~r/^"(?<entity>[^"]+)" in flight transaction most recently piggybacked output is not spendable any more$/,
         %{entity: entity},
         state do
    get_balance(state[entity]) |> IO.inspect(label: "#{entity} in flight transaction most recently piggybacked output is not spendable any more")
