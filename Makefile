...
test: ## run tests
test: compile
	mix test --no-start --include integration
...
operator: ## run child chain operator in dev mode; see also `geth`, `env` and `eth_block` targets
operator: compile
	cd apps/omisego_api; iex --sname main -S mix

watcher: ## run watcher in dev mode; see also `geth`, `env` and `eth_block` targets
watcher: compile
	cd apps/omisego_watcher; iex --sname main -S mix
