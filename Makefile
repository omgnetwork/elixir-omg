...
start-watcher:
	...
	_build/${BAREBUILD_ENV}/rel/watcher/bin/watcher eval "OMG.DB.ReleaseTasks.InitKeyValueDB.run_multi()" && \
	echo "Run Watcher" && \
	. ${OVERRIDING_VARIABLES} && \
...
start-watcher_info:
	...
	_build/${BAREBUILD_ENV}/rel/watcher_info/bin/watcher_info eval "OMG.DB.ReleaseTasks.InitKeyValueDB.run_multi()" && \
