        ...
        stage('Integration test') {
           withCredentials([string(credentialsId: 'elixir-omg_coveralls', variable: 'ELIXIR_OMG_COVERALLS')]){
                withEnv(["MIX_ENV=test", "SHELL=/bin/bash"]) {
                    ...
                    sh ("""
                        set +x
                        mix coveralls.post --token '${ELIXIR_OMG_COVERALLS}' --branch '${scmVars.GIT_BRANCH}' --name 'Jenkins' --sha '${scmVars.GIT_COMMIT}' --message '${commitMessage}' --umbrella --include integration --include wrappers
                    """)
                }
           }
        }
